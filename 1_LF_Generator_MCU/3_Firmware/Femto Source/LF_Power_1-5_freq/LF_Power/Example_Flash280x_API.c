...
SECTOR Sector[4] = {
         ...
         (Uint16 *)0x3F0000,(Uint16 *)0x3F001F,
...
#define ENC_KEY			0x55FF
Uint16 g_nFlashBufDec[FLASH_BUF_LEN];
Uint16 *g_pFlashStartPtr_B;	// Flash B.
Uint16 *g_pFlashStartPtr_C;	// Flash B.
Uint16 g_nRndKey = 0;
Uint32 g_nGenOnCount = 0;
Uint32 g_nGenTimeSec = 0;
extern Uint32 Flash_CPUScaleFactor;
extern Uint32 gnStartTimeVer;
extern Uint32 gnStartTimeLC;

int16 Ext_ExtractData (Uint16 buf[]);
int16 Ext_UpdateData (Uint16 buf[]);

//#pragma CODE_SECTION(Ext_Init_Flash,"ramfuncs");
int16 Ext_Init_Flash ()
{
	int16 i;
	Uint16 Status;
	Uint16 buf[FLASH_BUF_LEN];
	...
	//Example_CallFlashAPI_1();

	//...
	g_pFlashStartPtr_B = Sector[1].StartAddr + 0x2000;	// Flash B.
	g_pFlashStartPtr_C = Sector[2].StartAddr + 0x2000;	// Flash C.

	Example_MemCopy (g_pFlashStartPtr_B, g_pFlashStartPtr_B + FLASH_BUF_LEN, buf);

	/*
	if (Ext_DataUpdate(1) > 0)
	{
		Ext_DataConvert (0);
		Ext_Write_Flash (2);
	}
	...
	{
		Example_MemCopy (g_pFlashStartPtr_C, g_pFlashStartPtr_C + FLASH_BUF_LEN, g_nFlashBufDec);
		if (Ext_DataUpdate(1) > 0)
		{
			Ext_DataConvert (0);
			Ext_Write_Flash (1);
	...
	*/
	...
	if (Ext_ExtractData(buf) > 0)
	{
		Ext_Write_Flash (2);
	}
	else
	{
		Example_MemCopy (g_pFlashStartPtr_C, g_pFlashStartPtr_C + FLASH_BUF_LEN, buf);
		if (Ext_ExtractData(buf) > 0)
		{
			Ext_Write_Flash (1);
		}
		...
		{
			g_nFlashBufDec[0] = (Uint16)(gnStartTimeVer % 30000);
			g_nFlashBufDec[1] = g_nGenOnCount;
			g_nFlashBufDec[2] = g_nGenTimeSec / 30000;
			g_nFlashBufDec[3] = g_nGenTimeSec % 30000;
			g_nFlashBufDec[4] = gnStartTimeLC % 0x5100;
			g_nFlashBufDec[5] = gnStartTimeLC % 0x5200;
			g_nFlashBufDec[6] = gnStartTimeLC % 0x5300;

			i = 8;
			g_nFlashBufDec[i++] = 0xFFFD;
			g_nFlashBufDec[i++] = 0xFFFD;
			g_nFlashBufDec[i++] = 0xFFFD;
			g_nFlashBufDec[i++] = 0xFFFD;
			g_nFlashBufDec[i++] = 0xFFFE;
			g_nFlashBufDec[i++] = 0xFFFE;
			g_nFlashBufDec[i++] = 0xFFFE;
			g_nFlashBufDec[i++] = 0xFFFE;
...
#pragma CODE_SECTION(Ext_Write_Flash,"ramfuncs");
int16 Ext_Write_Flash (int16 flash_id)
{
	Uint16  Status;
	...
	if (flash_id == 1)
		Status = Flash_Program (g_pFlashStartPtr_B, buf, FLASH_BUF_LEN, &FlashStatus);
	else if (flash_id == 2)
		Status = Flash_Program (g_pFlashStartPtr_C, buf, FLASH_BUF_LEN, &FlashStatus);
	...
	if (flash_id == 1)
		Status = Flash_Verify (g_pFlashStartPtr_B, buf, FLASH_BUF_LEN, &FlashStatus);
	else if (flash_id == 2)
		Status = Flash_Verify (g_pFlashStartPtr_C, buf, FLASH_BUF_LEN, &FlashStatus);
...
//#pragma CODE_SECTION(Ext_DataConvert,"ramfuncs");
int16 Ext_DataConvert (int16 dir)
{
	if (dir <= 0)
	{
		g_nGenOnCount = g_nFlashBufDec[0] * 1000 + g_nFlashBufDec[1];
		g_nGenTimeSec = g_nFlashBufDec[2] * 10000 + g_nFlashBufDec[3];

		if (g_nFlashBufDec[0] == 0xFFFF && g_nFlashBufDec[1] == 0xFFFF)
			g_nGenOnCount = 0;

		if (g_nFlashBufDec[2] == 0xFFFF && g_nFlashBufDec[3] == 0xFFFF)
			g_nGenTimeSec = 0;
	}
	else
	{
		...
		for (i = 0; i < FLASH_BUF_LEN; i++)
			g_nFlashBufDec[i] = 0xFFFF;

		g_nFlashBufDec[FLASH_BUF_LEN - 1] = 0xFFFE;
		g_nFlashBufDec[FLASH_BUF_LEN - 2] = 0xFFFE;
		g_nFlashBufDec[FLASH_BUF_LEN - 3] = 0xFFFE;
		g_nFlashBufDec[FLASH_BUF_LEN - 4] = 0xFFFE;
		g_nFlashBufDec[FLASH_BUF_LEN - 1] = 0xFFFD;
		g_nFlashBufDec[FLASH_BUF_LEN - 2] = 0xFFFD;
		g_nFlashBufDec[FLASH_BUF_LEN - 3] = 0xFFFD;
		g_nFlashBufDec[FLASH_BUF_LEN - 4] = 0xFFFD;

		g_nGenCount++;

		g_nFlashBufDec[0] = g_nGenCount / 1000;
		g_nFlashBufDec[1] = g_nGenCount % 1000;

		g_nFlashBufDec[2] = g_nGenTime / 10000;
		g_nFlashBufDec[3] = g_nGenTime % 10000;

		g_nFlashBufDec[4] = (g_nFlashBufDec[0] / 2 + 1) + (g_nFlashBufDec[1] / 2 + 2);
		g_nFlashBufDec[5] = (g_nFlashBufDec[0] % 256 + 1) + (g_nFlashBufDec[1] % 256 + 2);

		g_nFlashBufDec[6] = (g_nFlashBufDec[2] / 2 + 1) + (g_nFlashBufDec[3] / 2 + 2);
		g_nFlashBufDec[7] = (g_nFlashBufDec[2] % 256 + 1) + (g_nFlashBufDec[3] % 256 + 2);
		*/
		Ext_DataUpdate (0);
	}
...
//#pragma CODE_SECTION(Ext_ExtractData,"ramfuncs");
int16 Ext_ExtractData (Uint16 buf[])
{
	int16 i, n;
	Uint16 cs = 0;
	...
	// decoding.
	for (i = 0; i < FLASH_BUF_LEN; i++)
		g_nFlashBufDec[i] = buf[i];

	for (i = 6; i > 0; i--)
		g_nFlashBufDec[i] = g_nFlashBufDec[i] ^ g_nFlashBufDec[i - 1];

	g_nFlashBufDec[0] = g_nFlashBufDec[0] ^ ENC_KEY;

	//...
	g_nRndKey = g_nFlashBufDec[0];
	g_nGenOnCount = g_nFlashBufDec[1];
	g_nGenTimeSec = g_nFlashBufDec[2] * 30000 + g_nFlashBufDec[3];

	return 1;
}

//#pragma CODE_SECTION(Ext_UpdateData,"ramfuncs");
int16 Ext_UpdateData (Uint16 buf[])
{
	int16 i, n;

	for (i = 0; i < FLASH_BUF_LEN; i++)
		buf[i] = g_nFlashBufDec[i];

	buf[0] = g_nRndKey;
	buf[1] = (Uint16)g_nGenOnCount;
	buf[2] = g_nGenTimeSec / 30000;
	buf[3] = g_nGenTimeSec % 30000;

	if (buf[4] == 0 || buf[5] == 0 || buf[6] == 0)
	{
		buf[4] = gnStartTimeVer % 30000;
		buf[5] = gnStartTimeLC % 30000;
		buf[6] = (gnStartTimeVer + gnStartTimeLC) % 30000;
		//buf[4] = 0;
		//buf[5] = 0;
		//buf[6] = 0;
	}
...
int16 Ext_DataUpdate (int16 is_check)
{
	int i;
	int16 cv_4, cv_5, cv_6, cv_7;

	if (is_check > 0)
	{
		cv_4 = (g_nFlashBufDec[0] / 2 + 1) + 	(g_nFlashBufDec[1] / 2 + 2);
		cv_5 = (g_nFlashBufDec[0] % 256 + 3) + (g_nFlashBufDec[1] % 256 + 4);
		cv_6 = (g_nFlashBufDec[2] / 2 + 5) + 	(g_nFlashBufDec[3] / 2 + 6);
		cv_7 = (g_nFlashBufDec[2] % 256 + 7) + (g_nFlashBufDec[3] % 256 + 8);

		//goto Ext_DataUpdate_Cal_CV;

//Ext_DataUpdate_Check:
		if (g_nFlashBufDec[4] != cv_4 || g_nFlashBufDec[5] != cv_5 ||
			g_nFlashBufDec[6] != cv_6 || g_nFlashBufDec[7] != cv_7)
			return -1;

		for (i = 8; i < 12; i++)
		{
			if (g_nFlashBufDec[i] != 0xFFFD)
				return -2;
		}

		for (i = 12; i < 15; i++)
		{
			if (g_nFlashBufDec[i] != 0xFFFE)
				return -3;
		}
	}
	else
	{
		...
		g_nFlashBufDec[0] = g_nGenOnCount / 1000;
		g_nFlashBufDec[1] = g_nGenOnCount % 1000;

		g_nFlashBufDec[2] = g_nGenTimeSec / 10000;
		g_nFlashBufDec[3] = g_nGenTimeSec % 10000;
...
//Ext_DataUpdate_Update:
		g_nFlashBufDec[4] = cv_4;
		g_nFlashBufDec[5] = cv_5;

		g_nFlashBufDec[6] = cv_6;
		g_nFlashBufDec[7] = cv_7;
	}
/*
	goto Ext_DataUpdate_Return;

Ext_DataUpdate_Cal_CV:
	cv_4 = (g_nFlashBufDec[0] / 2 + 1) + 	(g_nFlashBufDec[1] / 2 + 2);
	cv_5 = (g_nFlashBufDec[0] % 256 + 3) + (g_nFlashBufDec[1] % 256 + 4);
	cv_6 = (g_nFlashBufDec[2] / 2 + 5) + 	(g_nFlashBufDec[3] / 2 + 6);
	cv_7 = (g_nFlashBufDec[2] % 256 + 7) + (g_nFlashBufDec[3] % 256 + 8);
	...
	if (is_check <= 0)
		goto Ext_DataUpdate_Check;
	else goto Ext_DataUpdate_Update;

Ext_DataUpdate_Return:
*/
	return 1;
}


#pragma CODE_SECTION(Example_CallFlashAPI_1,"ramfuncs");
void Example_CallFlashAPI_1(void)
{
	   Uint16  i;
	   Uint16  Status;
	   ...
	   /*
	   Status = Flash_Erase((SECTORC | SECTORB),&FlashStatus);
	   if(Status != STATUS_SUCCESS)
	   {
	       Example_Error(Status);
	   }
