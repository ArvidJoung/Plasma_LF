...
int InitPwm_280x (int32 id, Uint16 ctrmode, Uint16 phsen, union AQCTL_REG aqctla, union AQCTL_REG aqctlb)
{
	...
	SetPwm_280x (id, ctrmode, 50000, 0);
...
void SetPwm_280x (int32 id, Uint16 ctrmode, int32 pwm_freq, float32 pwm_width)
{
	...
	{
		freq = pwm_freq;
		if (freq < MIN_PWM_FREQ_280x) freq = MIN_PWM_FREQ_280x;
		if (freq > MAX_PWM_FREQ_280x) freq = MAX_PWM_FREQ_280x;
	}

	if (pwm_width >= 0)
	{
		width = pwm_width;
		if (width < MIN_PWM_WIDTH_280x) width = MIN_PWM_WIDTH_280x;
		if (width > MAX_PWM_WIDTH_LIMIT_280x) width = MAX_PWM_WIDTH_LIMIT_280x; // 삭제 필요.
	}
...
//	else tbprd = 150000000 / freq;
	if (ctrmode == TB_COUNT_UPDOWN) tbprd = 100000000 / (freq * 2);
	else tbprd = 100000000 / freq;
	int32 cmp_width = (int32)(tbprd * width / MAX_PWM_WIDTH_280x);

	if (cmp_width < 0) cmp_width = 0;
	if (cmp_width > tbprd * MAX_PWM_WIDTH_LIMIT_280x / MAX_PWM_WIDTH_280x) // 삭제 필요.
		cmp_width = tbprd * MAX_PWM_WIDTH_LIMIT_280x / MAX_PWM_WIDTH_280x; // 삭제 필요.
